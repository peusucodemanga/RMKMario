...
public class Goomba : MonoBehaviour
{
    ...
    [SerializeField] float velocidade = 1f,distancia = 0.2f;
    ...
    void Awake()
    {
        ...
        Pontos=PlayerPrefs.GetInt("PontoM", 0);
    }
        private void Start()
    {
        MortoM=false;
    ...
    private void OnTriggerEnter2D(Collider2D collision){
        if (collision.tag =="Player"){
            ...
            Pontos=Pontos+10;
            PlayerPrefs.SetInt("PontoM",Pontos);
        ...
        if (collision.gameObject.CompareTag("Espinho"))Destroy(gameObject);
