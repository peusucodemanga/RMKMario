...
public class Movimentacao1 : MonoBehaviour
{
    ...
    int ContadorVida;
    Animator animPlayer;
    [SerializeField]public bool MortoM = false;
    ...
    private void Awake()
    {
        ...
        ContadorVida=PlayerPrefs.GetInt("ContadorVida", 3);
    }
    private void Start()
    {
        MortoM=false;
    ...
    if(!MortoM){
        ContadorVida--;
        PlayerPrefs.SetInt("ContadorVida", ContadorVida);
        audioManager.PararBack();
        if(ContadorVida>0) audioManager.PlaySFX(audioManager.Morte);
        else audioManager.PlaySFX(audioManager.GameOver);
        ...
        if(ContadorVida>0) Invoke("Reinicia",2.5f);
        else Invoke("GameOver",4f);
    }
